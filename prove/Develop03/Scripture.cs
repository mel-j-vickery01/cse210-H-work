...
class Scripture
{
    ...
    Scripture()
    {
        _scripture.Add("1 We believe in God, the Eternal Father, and in His Son, Jesus Christ, and in the Holy Ghost.");
        _scripture.Add("2 We believe that men will be punished for their own sins, and not for Adam’s transgression.");
        _scripture.Add("3 We believe that through the Atonement of Christ, all mankind may be saved, by obedience to the laws and ordinances of the Gospel.");
        _scripture.Add("4 We believe that the first principles and ordinances of the Gospel are: first, Faith in the Lord Jesus Christ; second, Repentance; third, Baptism by immersion for the remission of sins; fourth, Laying on of hands for the gift of the Holy Ghost.");
    ...
    Scripture(string bookName, int chapter, int verse, string text)
    {
        
    }
    Scripture(string bookName, int chapter, int startVers, int endVers, string text)
    {
        string[] words = (text, " ");
    }
